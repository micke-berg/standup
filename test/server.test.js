// Server security guards: DNS-rebinding, CSRF, and static path safety (pure; the module only
// starts a socket when run directly, so requiring it here has no side effects).
const { test } = require("node:test");
const assert = require("node:assert/strict");
const srv = require("../server.js");

test("hostAllowed: localhost forms pass, everything else fails", () => {
  assert.equal(srv.hostAllowed({ headers: { host: "localhost:7880" } }), true);
  assert.equal(srv.hostAllowed({ headers: { host: "127.0.0.1:7880" } }), true);
...
test("csrfSafe: POST that is same-origin / none / header-less (curl) is allowed", () => {
  ...
  assert.equal(srv.csrfSafe({ method: "POST", headers: { origin: "http://localhost:7880" } }), true);
});
...
test("staticFileFor: real files served, secrets + traversal rejected", () => {
  ...
  assert.equal(srv.staticFileFor("/app.js").status, 200);
  assert.equal(srv.staticFileFor("/app.css").status, 200);
